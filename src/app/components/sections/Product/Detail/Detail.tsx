...
export default function Detail({ details }: DetailProps) {
  return (
    <div>
      {/*Company*/}
      <span className={`lightText ${styles.company}`}>{details.company}</span>

      {/*Name*/}
      <h1 className={styles.name}>{details.name}</h1>

      {/*Description*/}
      <p className={`lightText ${styles.description}`}>{details.description}</p>
      ...
      <div className={styles.pricesCont}>
        ...
        {details.originalPrice ? (
          <div>
            ...
            <span className={`lightText ${styles.originalPrice}`}>
              ${details.originalPrice}
            </span>
          </div>
        ) : null}
